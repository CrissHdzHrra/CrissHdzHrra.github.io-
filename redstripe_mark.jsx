...
// --- CONFIGURACIÓN ---
const WHATSAPP_NUMBER = "526182384062"; // Número actualizado
...
const PRODUCTS = [
  ...
  { id: 6, name: "T-Shirt Gráfica 'Void'", category: "Ropa", subcategory: "Hombre", price: 350.00, img: "https://images.unsplash.com/photo-1576566588028-4147f3842f27?auto=format&fit=crop&q=80&w=600", tag: "BEST", sizes: ["XS","S","M","L","XL"], colors: ["#000", "#FFF"], rating: 4.7 },
...
const SEARCH_HISTORY_MOCK = ["Hoodies negras", "Diseño de logos", "Posters cyberpunk", "Camisas formales"];
...
export default function App() {
  ...
  const [chatMessages, setChatMessages] = useState([
    { id: 1, sender: 'admin', text: '¡Hola! Recibí tu solicitud de comisión. ¿Tienes referencias?', time: '10:00 AM' }
  ]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
        if (firebaseUser) {
            ...
            // Asegurar que existe admin
            try {
                const adminRef = doc(db, 'artifacts', appId, 'public', 'data', USERS_COLLECTION, 'admin');
                const adminSnap = await getDoc(adminRef);
                if (!adminSnap.exists()) {
                    await setDoc(adminRef, {
                        name: 'Administrador',
                        email: 'admin', // Se usa como identificador
                        password: 'admin',
                        role: 'admin',
  ...
  const handleSendMessage = (text, img = null) => {
    ...
    setTimeout(() => {
        setChatMessages(prev => [...prev, { id: Date.now() + 1, sender: 'admin', text: 'Entendido, lo revisaré pronto.', time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }]);
    }, 2000);
  };
...
const RedstripeBot = ({ setView, activateSearch }) => {
    ...
    const [messages, setMessages] = useState([
        { type: 'bot', text: '¡Hola! Soy el asistente de Redstripe Mark. ¿En qué te puedo ayudar hoy? Escribe algo o elige una opción.' }
    ]);
    ...
    const processMessage = (text) => {
        ...
        if (lowerText.includes("comisi") || lowerText.includes("diseño") || lowerText.includes("logo")) {
            botResponse = { type: 'bot', text: '¡Entendido! Si buscas servicios de diseño o arte, nuestro apartado de Comisiones es el lugar ideal. Te llevo allí.' };
            setView('commissions');
            ...
            botResponse = { type: 'bot', text: 'Para encontrar la prenda perfecta, te activo nuestra barra de búsqueda superior. ¡Adelante!' };
            activateSearch();
            ...
            botResponse = { 
                type: 'bot', 
                text: 'Claro, aquí tienes nuestras vías de contacto directo.',
                isContact: true 
            };
        } else {
    ...
    const handleOption = (option) => {
        ...
        setTimeout(() => {
            ...
            if (option === 'Busco comisiones') {
                ...
                botResponse = { type: 'bot', text: 'Claro, usa nuestra barra de búsqueda superior.' };
                activateSearch();
                     ...
                     <div className="p-4 bg-gray-50 flex-1 overflow-y-auto space-y-3 custom-scrollbar">
                         ...
                         {showOptions && (
                             <div className="grid grid-cols-1 gap-2 mt-2">
                                 <button onClick={() => handleOption('Busco comisiones')} className="text-left px-3 py-2 text-xs font-bold border rounded bg-white hover:bg-gray-50 text-gray-600">🎨 Busco comisiones</button>
                                 <button onClick={() => handleOption('Busco una prenda')} className="text-left px-3 py-2 text-xs font-bold border rounded bg-white hover:bg-gray-50 text-gray-600">👕 Busco una prenda</button>
                                 <button onClick={() => handleOption('Contacto / Info')} className="text-left px-3 py-2 text-xs font-bold border rounded bg-white hover:bg-gray-50 text-gray-600">ℹ️ Contacto / Info</button>
                             </div>
                         )}
                     </div>
        ...
        <div className="space-y-12 pb-20">
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-[#111] rounded-2xl p-8 text-white relative overflow-hidden group cursor-pointer h-64 md:h-80 flex flex-col justify-end">
                        <div className="relative z-10 transform translate-y-2 group-hover:translate-y-0 transition-transform">
                            ...
                            <p className="text-sm text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity mt-2">Nueva colección 2024 disponible.</p>
                        </div>
                    ...
                    <div className="bg-[#111] rounded-2xl p-8 text-white relative overflow-hidden group cursor-pointer h-64 md:h-80 flex flex-col justify-end">
                        <div className="relative z-10 transform translate-y-2 group-hover:translate-y-0 transition-transform">
                            ...
                            <p className="text-sm text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity mt-2">Logos, Overlays y más.</p>
                        </div>
            ...
            <section>
                <div className="flex items-end justify-between mb-8 border-b border-gray-200 pb-4">
                    <div>
                        <h2 className="text-4xl font-bebas">Catálogo Completo</h2>
...
const PortfolioView = () => (
    <div className="max-w-4xl mx-auto fade-in text-center py-10">
        ...
        <p className="text-gray-500 max-w-2xl mx-auto mb-16 text-lg">
            Una colección selecta de trabajos realizados para marcas urbanas, streamers y proyectos personales.
            Cada pieza cuenta una historia visual única.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
            ...
            <div className="space-y-8 pt-12">
                 ...
                 <div className="bg-gray-100 rounded-2xl p-12 flex items-center justify-center text-center">
                    <div>
                        <h3 className="font-bebas text-4xl mb-2">¿Te gusta lo que ves?</h3>
                        <p className="text-gray-500 mb-6">Trabajemos juntos.</p>
                        <button className="btn-primary w-full">Solicitar Comisión</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden flex flex-col md:flex-row scale-up">
                ...
                <div className="w-full md:w-1/2 p-8 flex flex-col justify-center">
                    <div className="flex items-center gap-2 mb-2">
                        <span className="bg-[var(--primary-red)] text-white text-[10px] px-2 py-0.5 rounded font-bold uppercase">Vista Rápida</span>
    ...
    const handleCheckout = () => {
        ...
        setTimeout(() => {
            const newOrder = {
                ...
                arrival: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000).toLocaleDateString(), // 5 días después
                items: [...cart],
                total: total,
                status: 'En camino'
            };
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
                    ...
                    {mode === 'cart' && activeTab === 'orders' && (
                        <div className="space-y-6">
                            {orders.length === 0 ? <p className="text-center text-gray-400 mt-10">Aún no tienes pedidos.</p> : orders.map(order => (
                        ...
                        <div className="space-y-6 fade-in">
                            ...
                            {checkoutStep === 1 ? (
                                <div className="space-y-4">
                                    ...
                                    <input placeholder="Dirección Completa" className="input-minimal" onChange={e => setFormData({...formData, address: e.target.value})} />
                                    ...
                                    <div className="flex gap-2 mt-4">
                                        <button className="flex-1 py-3 border border-gray-300 rounded hover:border-black font-medium text-sm">Estándar (Gratis)</button>
                                ...
                                <div className="space-y-4 fade-in">
                                    ...
                                    <div className="bg-gray-100 p-4 rounded-lg flex items-center gap-3 mb-4">
                                        <CreditCard size={20}/>
                                        <input placeholder="Número de Tarjeta" className="bg-transparent outline-none flex-1 text-sm" />
                                    </div>
                                    <div className="flex gap-4">
                    ...
                    {mode === 'success' && (
                        <div className="h-full flex flex-col items-center justify-center text-center fade-in">
                            ...
                            <p className="text-gray-500 text-sm mb-8 px-8">Hemos recibido tu pedido correctamente. Puedes rastrearlo en la sección de "Rastreo".</p>
            ...
            <div className="space-y-5">
                ...
                <div>
                    ...
                    <div className="border-2 border-dashed border-gray-200 rounded-lg p-8 flex flex-col items-center justify-center text-gray-400 hover:border-[var(--primary-red)] hover:text-[var(--primary-red)] transition-colors cursor-pointer bg-gray-50">
                        ...
                        <span className="text-xs font-bold">Click para subir o arrastra aquí</span>
                    </div>
                </div>
            ...
            <div className="flex-1">
                {tab === 'details' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold mb-4 flex items-center gap-2"><User size={18}/> Información Personal</h3>
                            <div className="space-y-4">
                                ...
                                <div><p className="text-xs text-gray-400 uppercase">País</p><p className="font-medium">{user.country || 'No especificado'}</p></div>
                            </div>
                            <button onClick={logout} className="mt-8 text-red-500 text-xs font-bold uppercase flex items-center gap-2 hover:bg-red-50 p-2 rounded w-fit"><LogOut size={14}/> Cerrar Sesión</button>
                        </div>
                ...
                {tab === 'wishlist' && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {wishlist.length === 0 ? <p className="text-gray-400">Tu lista de deseos está vacía.</p> : wishlist.map(p => (
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 h-[500px] flex flex-col shadow-sm overflow-hidden">
                        <div className="p-4 border-b border-gray-100 bg-gray-50 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-green-600 flex items-center gap-1"><span className="w-1.5 h-1.5 bg-green-500 rounded-full"></span> En línea</p>
                            </div>
                        </div>
...
const ModernAuth = ({ onLogin, isFirebaseReady }) => {
  const [isLogin, setIsLogin] = useState(true);
  const [formData, setFormData] = useState({ email: '', password: '', name: '' });
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);

  // Función para referencia segura del doc de usuario
  const getUserDocRef = (emailOrUser) => {
      // Normalizar: lower case, trim
      const safeId = emailOrUser.toLowerCase().trim();
      return doc(db, 'artifacts', appId, 'public', 'data', USERS_COLLECTION, safeId);
  };

  const handleSubmit = async () => {
      if (!isFirebaseReady) {
          setError("Conectando con el servidor... Intenta en un momento.");
          return;
      }
      ...
      try {
          ...
          if (isLogin) {
              ...
              // --- REGISTRO ---
              if (userSnap.exists()) {
                  setError("Este usuario/correo ya está registrado.");
              } else {
                  const newUser = {
                      name: name || 'Usuario Nuevo',
                      email: email, // Usamos email como ID único
                      password: password,
                      role: 'client',
                      createdAt: new Date().toISOString()
                  };
    ...
    <div className="flex h-screen w-full bg-white overflow-hidden font-sans">
      ...
      >
        ...
        <div className="max-w-md w-full fade-in space-y-8 relative z-10">
            ...
            {error && (
                <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-3 text-sm rounded animate-pulse">
                    <p>{error}</p>
                </div>
            )}
            ...
            <div className="space-y-5">
                ...
                <div className="space-y-1">
                    <label className="text-xs font-bold text-gray-600 uppercase ml-1">{isLogin ? "Usuario o Correo" : "Correo Electrónico"}</label>
                    <div className="relative">
                        <Mail className="absolute left-4 top-3.5 text-gray-500" size={18} />
                        <input 
                            placeholder={isLogin ? "usuario@ejemplo.com o admin" : "usuario@ejemplo.com"}
                ...
                <div className="space-y-1">
                    <label className="text-xs font-bold text-gray-600 uppercase ml-1">Contraseña</label>
                    <div className="relative">
                ...
                <button 
                    ...
                    disabled={loading}
                    className="w-full bg-black text-white py-4 rounded-lg font-bold text-sm hover:bg-[var(--primary-red)] transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center"
                >
                    {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : (isLogin ? "INICIAR SESIÓN" : "REGISTRARSE")}
                </button>

                <div className="relative flex py-2 items-center">
                    <div className="flex-grow border-t border-gray-300"></div>
                    <span className="flex-shrink-0 mx-4 text-gray-500 text-xs uppercase">O continúa con</span>
                    <div className="flex-grow border-t border-gray-300"></div>
                </div>
                ...
                <div className="text-center mt-6">
                    <button onClick={() => { setIsLogin(!isLogin); setError(''); }} className="text-sm font-medium hover:text-[var(--primary-red)] transition-colors underline decoration-dotted underline-offset-4">
